...
void VulkanRenderer::initVulkan() {
    ...
    {
        createTextureDescriptorSet(texture);
    }
...
void VulkanRenderer::cleanupSwapChain() {
    ...
    for (size_t i = 0; i < swapChainImages.size(); i++) {
        vkDestroyBuffer(device, cameraBuffers[i].bufferID, nullptr);
        vkFreeMemory(device, cameraBuffers[i].bufferMemoryID, nullptr);
        vkDestroyBuffer(device, gLightsBuffers[i].bufferID, nullptr);
        vkFreeMemory(device, gLightsBuffers[i].bufferMemoryID, nullptr);
    }
...
void VulkanRenderer::createDescriptorSetLayout() {
    ...
    gLightsLayoutBinding.descriptorCount = 1;
    gLightsLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    gLightsLayoutBinding.pImmutableSamplers = nullptr;
    gLightsLayoutBinding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT;

    std::array<VkDescriptorSetLayoutBinding, 2> bindings = { cameraLayoutBinding,gLightsLayoutBinding };
...
void VulkanRenderer::CreateGraphicsPipeline(const char* vertSPV, const char* fragSPV, const char* geoSPV) {
    ...
    // TODO::PUSH_CONSTANTS:  Set Up Push Constant Range
    ...
    // TODO::PUSH_CONSTANTS: Set Up Push Constant Pipeline Layout Info
...
void VulkanRenderer::createCameraUBO() {
    ...
    for (size_t i = 0; i < swapChainImages.size(); i++) {
        createBuffer(bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, 
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
            cameraBuffers[i].bufferID, cameraBuffers[i].bufferMemoryID);
    }
}

void VulkanRenderer::createGLightsUBO() {
    VkDeviceSize bufferSize = sizeof(GLightsUBO);
    gLightsBuffers.resize(swapChainImages.size());
    for (size_t i = 0; i < swapChainImages.size(); i++) {
        createBuffer(bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
            gLightsBuffers[i].bufferID, gLightsBuffers[i].bufferMemoryID);
    }
}
...
void VulkanRenderer::createDescriptorPool() {
    ...
    poolSizes[2].descriptorCount = 2; //static_cast<uint32_t>(swapChainImages.size());
...
void VulkanRenderer::createDescriptorSets() {
    ...
    for (size_t i = 0; i < swapChainImages.size(); i++) {
        ...
        gLightsBufferInfo.buffer = gLightsBuffers[i].bufferID;
        gLightsBufferInfo.offset = 0;
        gLightsBufferInfo.range = sizeof(GLightsUBO);
...
void VulkanRenderer::createTextureDescriptorSet(Texture& texture)
{
    ...
    descriptorWrite.dstSet = texture.descriptorSet; //descriptorSets[i];
...
void VulkanRenderer::SetGLightsUBO(const int& index, const Vec4& position, const Vec4& diffuse, const Vec4& specular, const Vec4& ambient) {
    gLightsUBO.position[index] = position;
    gLightsUBO.diffuse[index] = diffuse;
    gLightsUBO.specular[index] = specular;
    gLightsUBO.ambient[index] = ambient;
    gLightsUBO.lightCount = index + 1;
}

// TODO::PUSH_CONSTANTS: SetMeshPushConstant implementation
...
void VulkanRenderer::updateCameraUBO(uint32_t currentImage) {
    ...
    vkUnmapMemory(device, cameraBuffers[currentImage].bufferMemoryID);
}

void VulkanRenderer::updateGLightsUBO(uint32_t currentImage) {
    void* data;
