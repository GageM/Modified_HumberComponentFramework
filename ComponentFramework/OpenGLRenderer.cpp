...
SDL_Window* OpenGLRenderer::CreateWindow(std::string name_, int width_, int height_)
{
	...
	if (window == nullptr)
	{
		Debug::FatalError("Failed to initialize SDL window", __FILE__, __LINE__);
		return nullptr;
	}
	return window;
}
...
void OpenGLRenderer::setAttributes(int major_, int minor_)
{
	...
	// Give SDL The Major & Minor Glew Version
	...
	// Set The Depth Buffer to be 32 Bit
...
void OpenGLRenderer::getInstalledOpenGLInfo(int* major, int* minor)
{
	...
	glGetIntegerv(GLEW_VERSION_MAJOR, major);
	glGetIntegerv(GLEW_VERSION_MINOR, minor);
