...
public:
	LightActor(Ref<Component> parent_, Ref<Renderer> renderer_, LightStyle lightStyle_, Vec3 position_, Vec4 colour_,
		float intensity_ = 1.0f, Vec3 fallOff_ = Vec3(0.0f, 0.0f, 0.0f));
	LightActor(Ref<Component> parent_, Ref<Renderer> renderer_);
	~LightActor();
